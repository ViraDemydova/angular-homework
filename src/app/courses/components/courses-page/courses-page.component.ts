...
export class CoursesPageComponent implements OnInit, OnChanges, OnDestroy {
  ...
  init(): void {
    this.coursesListService
      .getCourseListItems(this.countToStart, this.createDate)
  ...
  onDeleteCourse(listItem: CoursesListItem): void {
    // Метод сервиса принимает id, а передается ему что-то непонятно String(listItem),
    // где listItem - это CoursesListItem
    // Почему бы просто не передать listItem.id?
    this.usersDeleteSubscription = this.coursesListService
      .deleteItem(String(listItem))
      .subscribe(() => {
        this.init();
      });
  }
  ...
  public onSearch(queryString: string): void {
    this.usersWithParamsSubscription = this.coursesListService
      .searchTextWithParams(queryString)
      .subscribe(
        (res: CoursesListItem[]) => {
          this.ListItems = res;
        },
  ...
  public createUser(data: object): void {
    this.usersCreateSubscription = this.coursesListService
      .addItem(data)
      .subscribe((res: CoursesListItem) => {
        this.init();
      });
  ...
  onLoadMore(): void {
    this.listItemsParamsSubscription = this.coursesListService
      .getCourseListItemsWithParams(this.countToLoad, this.createDate)
      .subscribe(
        (res: CoursesListItem[]) => {
          this.ListItems = res;
