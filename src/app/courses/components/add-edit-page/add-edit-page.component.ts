...
import {
  AddCourse,
  AddCourseSuccess
} from './store/add-course/actions/course.actions';
...
import {
  DateValidator,
  dateValidator
} from '../../../shared/directives/validator';
...
export class AddEditPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.authorCreateSubscription = this.courseService
      .getAuthors()
    ...
    if (this.indificator) {
      this.usersIdSubscription = this.courseService
        .getCourseById(this.indificator)
        .subscribe(
          (res: CoursesListItem) => {
            this.listItem = res;
            // Edit Course
  ...
  initEditForm() {
    ...
    this.editCourseForm = this.formBuilder.group({
      // По условию задачи тут должно быть ограничение в 50 симвлов, а не 30.
      title: [
        this.listItem.title,
      ...
      // Насколько я понял, то есть недопонимание по валидаторам.
      // Правило следующее
      // Если форма реактивная, то валидатор является функция, которая принимает контро и ее надо передать в массив
      // Если форма темпл-дривн, то валидатор - это директива. Она используется только в шаблоне
      duration: [this.listItem.duration, [Validators.required, numbersonly]],
      // В шаблоне уже есть директива валидации. Тут не нужен валидатор.
      // Если ты хочешь его тут использовать, то это должен быть не класс, а функция.
      // Например dateValidator
      // Формат даты должен быть другой
      createDate: [
        this.listItem.createDate,
  ...
  initAddForm() {
    ...
    this.addCourseForm = this.formBuilder.group({
      ...
      duration: [this.newItem.duration, [Validators.required]],
      createDate: [
        this.newItem.createDate,
        [Validators.required, DateValidator]
      ],
