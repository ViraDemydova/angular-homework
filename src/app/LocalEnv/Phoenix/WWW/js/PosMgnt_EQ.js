...
function submitPage( frm , action, trancheIDtoUpdate, worksheetType )
{
var lTotalFreeRetention = new Number(frm.iTxtFreeRetention.value.replace(/(\,)/g, ""));
var lStreet = new Number(frm.iTxtStreet.value.replace(/(\,)/g, ""));
switch (action)
{
case "savechanges" :
if(ValidateForm( frm ))
...
if (worksheetType == 'Target')
{
frm.hidProgID.value = 'PositionManagement_usr.TargetAllocation';
}
frm.submit(); 
}
break;
